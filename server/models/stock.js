...
const sellStock = async(email, symbol, number) => {
  try{
    const quote = await getStockQuote(symbol)
    const totalPriceMinus = Decimal128.fromString(`${(number * quote.latestPrice).toFixed(2)}`)
    await getDB().collection('users').updateOne({
      email: email
      }, {
        $inc: {
          [`portfolio.${symbol}`]: -number,
          cash: totalPriceMinus
        }
      })
      return {
...
module.exports = { getUserCash, getUserPortfolio, getUserTransactions, setUserShare, sellStock }